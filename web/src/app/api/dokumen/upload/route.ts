...
export async function POST(req: NextRequest) {
    try {
        ...
        const siswaId = formData.get("siswa_id") as number;

        if (!userId) return NextResponse.json({ success: false, message: "UserId wajib ada" }, { status: 400 });
        if (!siswaId) return NextResponse.json({ success: false, message: "SiswaId wajib ada" }, { status: 400 });
